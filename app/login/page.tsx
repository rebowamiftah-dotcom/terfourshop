...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    const cleanIdentity = data.identity.trim();
    const isUsername = cleanIdentity.startsWith("@");

    try {
      setIsLoading(true);

      if (isUsername) {
        // ALUR USERNAME: NextAuth Credentials
        ...
        router.push(`/login/verifikasi?email=${encodeURIComponent(cleanIdentity.toLowerCase())}`);
      }
  ...
  // HANDLER KETIKA VALIDASI ZOD GAGAL
